...
interface RewardPageProps {
  ...
  pronunciationScore?: number; // TAMBAHAN: Menerima skor pelafalan
  onGoHome: () => void;
}
...
export function RewardPage({ unitNumber, onContinue, isLastUnit, wrongAnswers = [], score = 0, totalQuestions = 0, pronunciationScore = 0, onGoHome }: RewardPageProps) {
  ...
  const getNewlyUnlockedTreasures = () => {
    ...
    // Lencana Kuis
    if (percentage >= 70 && unitNumber === 1) unlocked.push({ icon: Flame, label: 'Spark of Knowledge', requirement: 'Score 70%+ in 1 Expedition', color: 'text-rose-600', bgColor: 'from-rose-100 to-red-100' });
    if (percentage >= 80 && unitNumber === 2) unlocked.push({ icon: Medal, label: 'Silver Scholar', requirement: 'Score 80%+ in 2 Expeditions', color: 'text-slate-600', bgColor: 'from-slate-100 via-gray-100 to-slate-200' });
    ...
    if (percentage === 100 && unitNumber === 3) unlocked.push({ icon: Gem, label: 'Master Scholar', requirement: 'Score 100% in 3 Expeditions', color: 'text-fuchsia-600', bgColor: 'from-fuchsia-100 via-pink-100 to-purple-100' });
    ...
    // Lencana Pelafalan (Hanya memunculkan yang tertinggi untuk sesi ini agar layar tidak penuh)
    if (pronunciationScore >= 95) {
      unlocked.push({ icon: Gem, label: 'Diamond Whisperer', requirement: 'Get 95%+ in Pronunciation', color: 'text-cyan-500', bgColor: 'from-cyan-100 to-blue-200' });
    } else if (pronunciationScore >= 80) {
      unlocked.push({ icon: Mic, label: 'Golden Orator', requirement: 'Get 80%+ in Pronunciation', color: 'text-yellow-600', bgColor: 'from-yellow-100 to-yellow-300' });
    } else if (pronunciationScore >= 60) {
      unlocked.push({ icon: Mic, label: 'Silver Voice', requirement: 'Get 60%+ in Pronunciation', color: 'text-slate-500', bgColor: 'from-slate-100 to-slate-300' });
    } else if (pronunciationScore > 0) {
      unlocked.push({ icon: Mic, label: 'Brave Speaker', requirement: 'Complete speaking practice', color: 'text-orange-700', bgColor: 'from-orange-100 to-amber-200' });
    }
                ...
                <div className="space-y-3 sm:space-y-4">
                  ...
                  <div className="bg-white p-4 sm:p-5 rounded-xl border border-amber-200 shadow-sm">
                    ...
                    <div className="w-full h-4 sm:h-5 bg-amber-100 rounded-full overflow-hidden border border-amber-200 shadow-inner">
                      <motion.div initial={{ width: 0 }} animate={{ width: `${percentage}%` }} transition={{ duration: 1.2, delay: 1.4, ease: "easeOut" }} className={`h-full relative ${percentage === 100 ? 'bg-gradient-to-r from-yellow-400 to-amber-500' : percentage >= 80 ? 'bg-gradient-to-r from-blue-400 to-indigo-500' : 'bg-gradient-to-r from-red-400 to-rose-500'}`}>
                        <div className="absolute inset-0 bg-gradient-to-b from-white/40 to-transparent" />
                      </motion.div>
                    </div>
                  </div>

                  {/* BAR VOICE SCORE BARU */}
                  <div className="bg-white p-4 sm:p-5 rounded-xl border border-amber-200 shadow-sm">
                    <div className="flex justify-between items-center mb-3">
                      <span className="text-amber-800 font-[Nunito] font-bold text-sm sm:text-base">Voice Score:</span>
                      <span className={`font-bold font-[Fredoka] text-lg sm:text-xl ${pronunciationScore >= 80 ? 'text-green-600' : pronunciationScore >= 60 ? 'text-amber-600' : 'text-rose-600'}`}>
                        {pronunciationScore}%
                      </span>
                    </div>
                    <div className="w-full h-4 sm:h-5 bg-amber-100 rounded-full overflow-hidden border border-amber-200 shadow-inner">
                      <motion.div initial={{ width: 0 }} animate={{ width: `${pronunciationScore}%` }} transition={{ duration: 1.2, delay: 1.6, ease: "easeOut" }} className={`h-full relative ${pronunciationScore >= 80 ? 'bg-gradient-to-r from-green-400 to-emerald-500' : pronunciationScore >= 60 ? 'bg-gradient-to-r from-amber-400 to-orange-500' : 'bg-gradient-to-r from-rose-400 to-red-500'}`}>
                        <div className="absolute inset-0 bg-gradient-to-b from-white/40 to-transparent" />
                      </motion.div>
                    </div>
                  </div>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }} className="w-full">
              ...
              <div className={`grid ${earnedTreasures.length === 1 ? 'grid-cols-1 max-w-[200px] mx-auto' : 'grid-cols-2 sm:grid-cols-3 lg:grid-cols-2 xl:grid-cols-3'} gap-3 sm:gap-4`}>
                {earnedTreasures.map((badge, index) => (
                  <motion.div key={index} initial={{ opacity: 0, scale: 0 }} animate={{ opacity: 1, scale: 1 }} whileHover={{ scale: 1.05, y: -5 }} transition={{ delay: 0.8 + index * 0.1, type: "spring", stiffness: 150 }} className={`bg-gradient-to-br ${badge.bgColor} rounded-2xl p-4 sm:p-5 shadow-md border-2 border-white relative z-10 text-center flex flex-col items-center justify-center`} style={{ boxShadow: '0 4px 15px rgba(0,0,0,0.05)' }}>
